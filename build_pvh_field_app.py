...
(function(){
var DKEY="pvh_data", UKEY="pvh_data_url", CKEY="pvh_last_check", CHECK_MS=600000;
...
function fetchData(u){
  return fetch(directUrl(u),{cache:"no-store"}).then(function(r){
    if(!r.ok)throw new Error("HTTP "+r.status);
    return r.text();
  }).then(function(t){
    var d=JSON.parse(t);
    if(!d||!d.vehicles||!d.operators)throw new Error("not a PVH data file");
...
window.__checkNow=function(){check(true);};
window.__applyUpdate=function(){
  if(!PENDING){window.__closeToast();return;}
  lsSet(DKEY,PENDING.text);stampCheck();location.reload();
};
...
window.__datasrc=function(){
  ...
  mainEl().innerHTML='<div class="seclabel">Automatic data source</div>'+
    '<div class="notes">Put PVH_data.json in a OneDrive folder, copy its share link and paste it below. '+
    ...
    'In OneDrive use Share → Copy link. A personal 1drv.ms or onedrive.live.com link works as pasted. '+
    'A work or school link (…sharepoint.com…) only works if the tenant lets another site read the file — '+
    'if the check below fails with a network or CORS error, that is the reason.</div>'+
  ...
  document.getElementById("dssave").addEventListener("click",function(){
    ...
    fetchData(u).then(function(res){
      ...
      msg.textContent="Could not read that link: "+e.message+
        " — it has to point straight at PVH_data.json, and the host has to allow this site to read it.";
    });
  });
...
(function start(){
  ...
  document.addEventListener("visibilitychange",function(){
    if(document.visibilityState==="visible"&&dueForCheck())check(false);
  });
})();
})();"""
...
#toast{position:fixed;left:50%;transform:translateX(-50%);bottom:14px;z-index:60;
  width:calc(100% - 24px);max-width:560px;background:var(--panel);
