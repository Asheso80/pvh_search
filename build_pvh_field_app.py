...
def quality_report(vehicles, operators):
    ...
    for field, label in [("Plate No", "plate"), ("VIN", "VIN"),
                         ...
                         ("NSVehicle Permit Expiry", "permit expiry"),
...
def _veh_flags(v):
    ...
    if due and due < datetime.date.today():
        f.add("MVI")
    for field, label in [("NSVehicle Permit Expiry", "Permit"),
...
function showImport(msg){
  ...
  m.innerHTML='<div class="hint" style="padding-top:50px">'+(msg||"No data loaded on this device yet.")+'</div>'+
  ...
  document.getElementById("datafile").addEventListener("change",function(ev){
    ...
    r.onload=function(){
      try{
        ...
        try{localStorage.setItem("pvh_data",r.result);}catch(e){}
        boot(d);
...
(function(){
  ...
  var saved=null;
  try{saved=localStorage.getItem("pvh_data");}catch(e){}
  if(saved){
    try{boot(JSON.parse(saved));return;}
    catch(e){try{localStorage.removeItem("pvh_data");}catch(_){}}
  }
  showImport();
})();"""
...
.fchips{display:flex;gap:6px;overflow-x:auto;padding:2px 0 10px;
  -webkit-overflow-scrolling:touch;scrollbar-width:none}
.fchips::-webkit-scrollbar{display:none}
.fchip{flex:0 0 auto;border:1px solid var(--line);background:var(--panel);
  color:var(--dim);border-radius:20px;padding:7px 13px;font-size:13px;
  font-weight:600;cursor:pointer;white-space:nowrap}
.fchip.on{background:var(--accent);border-color:var(--accent);color:#fff}
.browsebar{display:flex;gap:8px;margin-bottom:10px}
.browsebar select{flex:1;height:42px;border:1px solid var(--line);border-radius:10px;
  background:var(--panel);color:var(--text);font-size:14px;padding:0 10px;
...
function vehFlagList(v,soon){
  ...
  chkInto(out,soon,"permit","Permit",v["NSVehicle Permit Expiry"]);
...
const FILTERS=[["all","All"],["permit","Permit"],["ins","Insurance"],["vlic","Veh licence"],
...
function vehSummary(v){
  return ["PVH VEHICLE \u2014 Deck "+(v["Deck No"]==null?"\u2014":v["Deck No"])+" / Plate "+(v["Plate No"]||"\u2014"),
    ...
    statusText("Permit",v["NSVehicle Permit Expiry"]),
...
const STAT_OPTS=[["","Any status"],["expired","Any expired"],["permit","Permit exp"],["ins","Insurance exp"],
  ...
  if(!r){
    ...
    main.innerHTML='<div class="hint">Search by deck light number, plate, owner or operator name, business or licence number.</div>'+
      ...
      (n?'<div class="card" onclick="go(\'flags\')">'+
        ...
        '<div class="csub">Expired or missing: permits, insurance, licences, MVI \u00B7 inactive operators</div></div>'+
...
function renderVehicle(i){
  ...
  let h='<div class="dhead"><div class="dtop">'+deckHTML(v,true)+
    ...
    status("Permit",v["NSVehicle Permit Expiry"])+
